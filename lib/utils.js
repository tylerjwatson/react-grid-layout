 ...
 */
export function compactNew(layout: Layout, type: CompactType, cols: number) {
  const compareWith = getStatics(layout);
  const sorted = sortLayoutItems(layout, type);

  for (const item of sorted) {
    if (!item.static) {
      compactItem(compareWith, item, type, cols, sorted);
 ...
 */
export function correctBounds(layout: Layout, bounds: { cols: number }): void {
 ...
 */
export function getLayoutItem(layout: Layout, id: any): ?LayoutItem {
  for (let i = 0, len = layout.length; i < len; i++) {
 ...
 *
 * @param  {Array}  layout Layout passed in through props.
 * @param  {String} breakpoint    Current responsive breakpoint.
 * @param  {?String} compact      Compaction option.
 * @return {Array}                Working layout.
 */
// TODO: Make mutate initialLayout
export function synchronizeLayoutWithChildren(
  ...
  React.Children.forEach(children, (child: ReactElement<any>) => {
    let itemLayout: ?LayoutItem = getLayoutItem(layout, child.key);
  ...
  compactNew(layout, compactType, cols);
}
